...
"""
    ...
    node (label) {
        stage ('Checkout SCM'){
          git credentialsId: 'git', url: 'https://gitspace.k4m.in/k4m_admin/eos-micro-services-admin.git', branch: 'master'
          container('build') {
        ...
        stage ('Sonar Scan'){
          container('build') {
                stage('Sonar Scan') {
                  ...
                  sh './mvnw verify org.sonarsource.scanner.maven:sonar-maven-plugin:sonar -Dsonar.projectKey=k4m-eos_eos'
                }
                }
            }
        }
        ...
        stage ('Artifactory configuration'){
          container('build') {
                stage('Artifactory configuration') {
                    rtServer (
                    id: "jfrog",
                    url: "https://k4meos.jfrog.io/artifactory",
                    credentialsId: "jfrog"
                )
